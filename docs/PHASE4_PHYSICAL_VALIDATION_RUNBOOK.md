...
This runbook defines the mandatory, step-by-step physical commissioning and calibration procedure for the Xiangqi robot. It provides an exhaustive testing ladder designed to transition the robot from uncalibrated hardware to fully autonomous, validated physical operation under the Phase 3B architecture.
...
4. **Phase 3B Architecture Authority:** All autonomous motion executes via `PieceMoveIntent` / `CaptureIntent` $\to$ `MotionResolver` $\to$ `MotionPlan` $\to$ `MotionExecutor` $\to$ `PhysicalFR3Backend`. Legacy `MotionCoordinator` is deprecated and retained strictly for backwards compatibility.
...
The system explicitly separates two distinct tool definitions. Never conflate calibration contact geometry with gameplay grasp geometry. Tool Frame IDs on the controller are assigned during commissioning (`TBD_PHYSICAL_VALIDATION`) and recorded in the validation log.
...
1. **`TCP_CALIBRATION` (Tool Frame ID `TBD`):**
...
4. **Record Findings:**
   ...
   * Current software default of $0.30\text{ s}$ is a provisional baseline; record actual empirical value.
...
The four teaching points R1, R2, R3, R4 correspond strictly to **outermost grid intersections** of the Xiangqi playing surface, **NOT** the physical wooden/plastic edges of the board. All coordinates follow the canonical `(row, col)` tuple convention:
...
* **R1:** Intersection at `(row=0, col=0)` — Black Left Chariot (Xe Đen Trái)
* **R2:** Intersection at `(row=0, col=8)` — Black Right Chariot (Xe Đen Phải)
* **R3:** Intersection at `(row=9, col=8)` — Red Right Chariot (Xe Đỏ Phải)
* **R4:** Intersection at `(row=9, col=0)` — Red Left Chariot (Xe Đỏ Trái)

Canonical Board-Local Coordinates ($Z=0$ at board surface, origin at board geometric center):
...
1. **Teaching Tool Active:** Mount calibrated pointer stylus. Activate configured `CALIBRATION_TOOL_ID` (`TCP_CALIBRATION`). Set user frame to Base Frame (`user_id = 0`).
...
### 6.2 Acceptance Thresholds & Authoritative Code Contracts
The validation thresholds are enforced directly by `BoardCalibrationTolerancePolicy` in `src/domain/board_pose_provider.py` (`AUTHORITATIVE_CODE_CONTRACT`):

| Metric | Canonical Contract | Warning Threshold | Hard Failure Threshold | Corrective Action on Failure |
| :--- | :--- | :--- | :--- | :--- |
| **Width (R1–R2, R4–R3)** | $320.00\text{ mm}$ | $|\Delta| \in (3.0, 5.0]\text{ mm}$ | $|\Delta| > 5.0\text{ mm}$ | Re-teach R1–R4. Verify pointer deflection. |
| **Height (R2–R3, R1–R4)** | $360.00\text{ mm}$ | $|\Delta| \in (3.0, 5.0]\text{ mm}$ | $|\Delta| > 5.0\text{ mm}$ | Re-teach R1–R4. Inspect board scaling. |
...
| **RMS Fit Residual** | $0.00\text{ mm}$ | $\text{RMS} \in (2.0, 3.0]\text{ mm}$| $\text{RMS} > 3.0\text{ mm}$ | Poor Kabsch rigid registration. Re-teach points. |
...
| **Board Tilt Angle** | $0.00^\circ$ | $\theta \in (2.5, 5.0]^\circ$ | $\theta > 5.0^\circ$ | Table/board is not level. Level base plate/table. |
...
*Provenance: `HISTORICAL_MEASUREMENT`. This value represents an unverified teaching point from an earlier prototype. It must be treated as a provisional reference, not validated truth.*
...
$$Z_{\text{safe}} = Z_{\text{board\_surface}} + 40.0\text{ mm} \quad (\text{CURRENT\_SOFTWARE\_DEFAULT})$$
...
* In software, `PICK_TCP_HEIGHT_MM = 4.715` is a `PROVISIONAL_SIMULATION / GEOMETRIC STARTING VALUE`.
...
Place a clean piece at cell `(row=4, col=4)`. Align gripper over cell at $Z_{\text{safe}}$. Open jaws fully.
...
1. Pick piece from `(row=4, col=4)`. Move to `(row=4, col=5)` at $Z_{\text{safe}}$.
...
5. **Transit:** Linear motion $\to (row=4, col=5, Z_{\text{safe}})$.
6. **Descend:** Linear descent $\to (row=4, col=5, Z_{\text{place}})$.
7. **Release:** Assert Open pulse ($t_{\text{open}}$). Wait $0.25\text{ s}$ settle time.
8. **Retreat:** Linear vertical retract $\to (row=4, col=5, Z_{\text{safe}})$.
...
*Use actual playable grid intersections, not outer table corners (strictly `(row, col)` convention):*
* **Corner 1 (R1 Region):** Cell `(row=0, col=0)` — Near-left boundary (Black Left Chariot)
* **Corner 2 (R2 Region):** Cell `(row=0, col=8)` — Near-right boundary (Black Right Chariot)
* **Corner 3 (R3 Region):** Cell `(row=9, col=8)` — Far-right boundary (Red Right Chariot)
* **Corner 4 (R4 Region):** Cell `(row=9, col=0)` — Far-left boundary (Red Left Chariot)

### 12.3 Execution Protocol
1. For each corner cell in order (C1 $\to$ C2 $\to$ C3 $\to$ C4):
   * Approach $(row, col, Z_{\text{safe}})$.
   * Descend to $(row, col, Z_{\text{pick}})$.
...
1. **Center:** `(row=4, col=4)` and `(row=5, col=4)` (Board midpoint / River)
2. **Top Edge (Black Baseline):** `(row=0, col=4)`
3. **Bottom Edge (Red Baseline):** `(row=9, col=4)`
4. **Left Edge:** `(row=4, col=0)`
5. **Right Edge:** `(row=4, col=8)`
...
## Section 15: Capture-Bin Calibration & Safety Interlock

### 15.1 Physical Provenance & Software Interlock
In current software contracts:
```python
CAPTURE_BIN_POSE_MM_DEG = (...)      # LEGACY_UNVERIFIED
CAPTURE_BIN_PROVENANCE = "LEGACY_UNVERIFIED"
CAPTURE_BIN_VALIDATED = False         # Safety interlock: blocks physical capture motion
```
Physical capture moves are **blocked by software interlock** until the physical bin location is taught, recorded, and validated.
...
### 16.1 Objective
Execute a complete, autonomous non-capture game move from perception/intent through physical execution and verification using the authoritative Phase 3B pipeline.

### 16.2 Sequence
Move: Red Cannon `(row=2, col=1)` $\to$ `(row=2, col=4)`
1. Place Red Cannon piece precisely at `(row=2, col=1)`.
2. Dispatch `PieceMoveIntent` through the authoritative pipeline:
   $$\text{PieceMoveIntent} \longrightarrow \text{MotionResolver} \longrightarrow \text{MotionPlan} \longrightarrow \text{MotionExecutor} \longrightarrow \text{PhysicalFR3Backend}$$
3. Verify step-by-step physical progression:
   * [ ] Pre-move safe height approach to source `(row=2, col=1, Z_{\text{safe}})`.
   * [ ] Clean vertical descent to $Z_{\text{pick}}$ and jaw closure.
   * [ ] Vertical lift to $Z_{\text{safe}}$ without tilting adjacent pieces.
   * [ ] Transit at $Z_{\text{safe}}$ along straight linear path.
   * [ ] Descent to destination `(row=2, col=4, Z_{\text{place}})`.
...
1. Place Black piece at `(row=0, col=0)`; place Red piece at `(row=9, col=0)`.
2. Dispatch `CaptureIntent` through `MotionResolver` and `MotionExecutor`:
   * **Phase 1 (Opponent Removal):**
     * Arm moves to destination `(row=9, col=0, Z_{\text{safe}})`.
     * Descends to $Z_{\text{pick}}$, grips Red piece, lifts to $Z_{\text{safe}}$.
   ...
   * **Phase 2 (Attacker Relocation):**
     * Arm transits to source `(row=0, col=0, Z_{\text{safe}})`.
     * Descends to $Z_{\text{pick}}$, grips Black piece, lifts to $Z_{\text{safe}}$.
     * Transits to destination `(row=9, col=0, Z_{\text{safe}})`.
...
```json
{
  ...
  "unit": "mm",
  "provenance_class": "PHYSICALLY_VALIDATED",
  ...
  "tool_id_used": "GRASP_TOOL_ID_TBD",
  "board_id": "STANDARDIZED_WOODEN_CC_01",
  "operator_signature": "Lead Commissioning Engineer",
