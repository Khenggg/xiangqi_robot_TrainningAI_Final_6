...
class FR5Robot:
    ...
    def connect(self):
        ...
        try:
            ...
            if err != 0:
                if self.board_mapping is not None:
                    raise RuntimeError(f"Robot mode failed: {err}")
    ...
    def board_to_pose(self, col, row, z_height, rotation=None):
        ...
        if self.board_mapping is not None:
            xy = self.board_mapping.xy(col, row)
            return xy + [z_height] + self._validated_rotation(config.ROTATION if rotation is None else rotation)
    ...
    def board_to_pose_bilinear(self, col, row, z_height, rotation=None):
        ...
        """
        if self.board_mapping is not None:
            return self.board_to_pose(col, row, z_height, rotation)
        try:
    ...
    def move_safe_pose(self, pose, speed=None, col=None, row=None):
        ...
        if err != 0 and (self.board_mapping is not None or err != 112):
    ...
    def gripper_ctrl(self, val):
        ...
        if err != 0:
            ...
            if self.board_mapping is not None:
                raise RuntimeError(f"Gripper command failed: {err}")
        return err
    ...
    def move_piece(self, s_col, s_row, d_col, d_row, is_capture,
        ...
        if self.board_mapping is not None:
            self.board_mapping.begin()
        elif getattr(config, "VISION_BACKEND", "yolo") == "cchess_qr" and not self.dry:
            raise RuntimeError("QR physical motion requires a measured camera/robot mapping")
        try:
            frozen_generation = self.board_mapping.frozen.generation if self.board_mapping is not None else None
            for name, target in (("moving", moving_visual_target), ("captured", captured_visual_target)):
                if target is not None and frozen_generation is not None:
                    if getattr(target, "generation", frozen_generation) != frozen_generation:
                        raise RuntimeError(f"Visual target {name} does not match current QR calibration")
            return self._move_piece_impl(s_col, s_row, d_col, d_row, is_capture,
                                         moving_visual_target, captured_visual_target)
        finally:
            if self.board_mapping is not None:
                self.board_mapping.end()

    def _move_piece_impl(self, s_col, s_row, d_col, d_row, is_capture,
                         moving_visual_target=None, captured_visual_target=None):
        ...
        if not self.connected and not self.dry:
            try:
                ...
                raise RuntimeError("Robot connection failed before move") from e
