...
class BoardRenderer:
    ...
    def draw_ui(self, game_state):
        ...
        # The physical arm runs in a worker; showing this explicitly makes it
        # clear the application is still responsive while motion is in progress.
        if game_state.get("robot_busy"):
            move_surf = self.ui_font.render("🤖  Robot is moving — controls locked", True, (255, 255, 255))
            padding = 10
            bg_rect = move_surf.get_rect(centerx=SCREEN_WIDTH // 2, top=8)
            bg_rect.inflate_ip(padding * 2, padding * 2)
            bg_surf = pygame.Surface((bg_rect.width, bg_rect.height), pygame.SRCALPHA)
            bg_surf.fill((0, 90, 180, 210))
            self.screen.blit(bg_surf, bg_rect.topleft)
            self.screen.blit(move_surf, move_surf.get_rect(center=bg_rect.center))
