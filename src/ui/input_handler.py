...
class InputHandler:
    ...
    def handle_mouse_down(self, mx, my):
        # Reset/surrender/manual input while the arm is moving could desync the
        # logical board from physical hardware. Window close remains handled by
        # main.py, but game controls wait for the motion worker to finish.
        if self.state.robot_busy:
            self.state.set_status("🤖 Robot đang di chuyển — thao tác bị khóa", color=(180, 100, 0), duration=2.0)
            return
    ...
    def handle_keyboard(self, key):
        if self.state.robot_busy or self.state.allow_mouse_move or self.state.game_over or self.state.turn != "r":
            return
    ...
    def _handle_space_key(self):
        if getattr(self.hw, "board_monitor", None) is not None:
            try:
                move = self.hw.detect_recognized_move(self.state.board, self.state.turn)
                if move is None:
                    raise RuntimeError("Bàn cờ chưa khớp đúng một nước đi hợp lệ")
                self.state.save_rollback_state()
                self.state.process_human_move(*move)
            except RuntimeError as exc:
                self.state.set_status(str(exc), color=(180, 100, 0), duration=5.0)
            return
