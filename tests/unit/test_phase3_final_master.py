...
class Phase3FinalMasterTests(unittest.TestCase):
    ...
    def test_bc7_negative_board_lowering_into_arm_rejected(self):
        ...
        # Generated by tools/simulation/find_board_lowering_collision_fixture.py
        # for canonical tool profile 150 mm on canonical physical geometry
        # initial clearance: 7.94mm (> 7.0mm), first blocking step: 2, min clearance: 4.75mm (robot_link_4)
        q_fixture = [0.0, -29.0, 146.0, -107.0, -90.0, 0.0]
    ...
    def test_c9_retreat_collision(self):
        ...
        q_ss = np.deg2rad(self.sim.backend.SERVICE_SAFE_JOINTS_DEG)
        tcp_ss_m = np.array(self.sim.backend._compute_tcp_pose_mm_deg(q_ss)[:3], dtype=float) / 1000.0
        p.resetBasePositionAndOrientation(obs.body_id, tcp_ss_m.tolist(), [0, 0, 0, 1])
